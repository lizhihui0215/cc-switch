...
## token4AI Provider 配置

token4AI 是 OpenAI-compatible provider，可通过 CC Switch 的本地代理和 Claude Code 路由接管使用。

基础信息：
- **Base URL**：`https://api.token4ai.cloud/v1`
- **模型发现 URL**：`https://api.token4ai.cloud/v1/models`
- **API 格式**：`OpenAI Responses API`
- **认证方式**：`Authorization: Bearer <token4AI API Key>`

配置步骤：

1. 切换到 **Claude Code** 应用，点击「添加供应商」。
2. 在供应商预设中选择 **token4AI**。
3. 输入 token4AI API Key。CC Switch 会沿用现有 API Key 输入、遮罩和存储逻辑。
4. 确认 **Base URL** 为 `https://api.token4ai.cloud/v1`。不要再额外追加 `/v1`。
5. 展开「高级选项」，确认 **API 格式** 为 **OpenAI Responses API**。
6. 点击「获取模型列表」，从下拉列表选择模型；如果模型列表为空或拉取失败，也可以手动输入模型名。
7. 启动本地代理，并为 **Claude Code** 启用「路由/代理接管」。
8. 运行 Claude Code。请求路径为：Claude Code → CC Switch local proxy → token4AI OpenAI-compatible endpoint。

Claude Code 环境变量示例（与 CC Switch 当前接管机制一致，使用 `ANTHROPIC_AUTH_TOKEN` 占位符）：

```bash
export ANTHROPIC_BASE_URL="http://127.0.0.1:<CC_SWITCH_PROXY_PORT>"
export ANTHROPIC_API_KEY=""
export ANTHROPIC_AUTH_TOKEN="cc-switch"
export ANTHROPIC_MODEL="<TOKEN4AI_SELECTED_MODEL>"
...
- **Base URL 重复 `/v1`**：正确模型发现 URL 是 `https://api.token4ai.cloud/v1/models`，不是 `https://api.token4ai.cloud/v1/v1/models`。
- **401/403**：检查 API Key 是否正确、是否有权限。
- **404**：检查 Base URL，尤其确认没有重复 `/v1`。
- **模型列表为空**：可以手动输入 token4AI 支持的模型名继续配置。
- **Tool calling 不支持**：会影响 Claude Code 的文件读取、编辑和执行工具能力，请换用支持工具调用的模型。
- **Streaming 异常**：优先更换模型；如供应商兼容性有差异，可尝试切换 API 格式后再测试。

## Token4AI 统一供应商

「Token4AI 统一供应商」是添加新供应商面板中的独立入口，可一次性配置 OpenAI/Codex、Anthropic/Claude、Gemini、MiniMax。四个产品分别使用独立 API Key，未填写 API Key 的产品会跳过；你也可以只配置其中一个产品。

默认 Base URL：

| 产品 | 默认 Base URL | 说明 |
|------|---------------|------|
| OpenAI / Codex | `https://api.token4ai.cloud/v1` | 用于 Codex；默认同时创建 Claude Code 兼容 provider，走 `openai_responses` / OpenAI-compatible 转换路径 |
| Anthropic / Claude | `https://api.token4ai.cloud` | 原生 Anthropic-compatible 路径，用于 Claude Code |
| Gemini | `https://api.token4ai.cloud` | 默认用于 Gemini CLI |
| MiniMax | `https://api.token4ai.cloud/v1` | 作为 OpenAI-compatible provider 创建；可手动尝试 Claude Code 兼容 |

配置步骤：

1. 点击「添加供应商」，切换到 **Token4AI 统一供应商** tab。
2. 按产品分别填写 API Key。每个产品的 API Key 独立保存，不会合并。
3. 按需调整 Provider 名称、Base URL 和模型名。模型名可以留空或手动输入。
4. OpenAI / Codex 和 MiniMax 支持通过 OpenAI-compatible `/models` 获取模型；失败时不会阻止保存。
5. 点击底部「添加」。CC Switch 只会为已启用且已填写 API Key 的产品创建 provider。
6. 创建成功后，在对应 app 的 provider 列表中选择 Token4AI provider 即可使用。

模型发现注意事项：

- OpenAI / Codex 的模型发现 URL 应为 `https://api.token4ai.cloud/v1/models`。
- MiniMax 的模型发现 URL 应为 `https://api.token4ai.cloud/v1/models`。
- 不要把 Base URL 写成 `https://api.token4ai.cloud/v1/v1`，否则会出现 `/v1/v1/models`。
- Anthropic / Claude 和 Gemini 不会强制走 OpenAI `/v1/models` 发现逻辑；如模型发现不可用，请手动输入模型名。

Claude Code 多模型兼容：

- **Token4AI OpenAI/GPT**：通过 `openai_responses` / OpenAI-compatible conversion path 支持 Claude Code，是推荐的 GPT 路径。
- **Token4AI Claude**：通过 Anthropic-compatible path 支持 Claude Code。
- **Token4AI MiniMax**：仅在复用现有 OpenAI-compatible conversion path 且模型支持 tool calling / streaming 时建议开启「尝试用于 Claude Code」。
- **Token4AI Gemini**：默认用于 Gemini CLI，不强制兼容 Claude Code。
